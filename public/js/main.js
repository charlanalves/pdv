...
//////////////////////// comum
    ...
    $(document).ready(function() {
      $('.notify').each(function (){
         if ($(this).hasClass('success')){
            notify('Sucesso', $(this).html(), 'success');
         }
         if ($(this).hasClass('error')){
            notify('Erro', $(this).html(), 'error');
         }
      })

   

       $('#selectUnidadeNegocio').multiselect({
      ...
      $('#selectUnidadeNegocio').on('change', function(){
         $("#divSelectUsuario").hide();
         $.ajax({
             url: base+'system/listarUsuarioUnidadeNegocio/'+$(this).val(),
             type:'GET',
             dataType:'json',
             success:function(resultado) {
                 if (resultado.status == true){
                     $('#selectUsuario').html(resultado.data);
                     alterarSelectUnidadeNegocio();
                     $("#divSelectUsuario").show();
                 }
             }
         })
      ...
      $('#btnAlterarUnidade').click(function(){
         var unidade = $('#selectUnidadeNegocio').val();
         var usuario = $('#selectUsuario').val();
         $.ajax({
             url: base+'system/alterarUsuario/'+unidade+'/'+usuario,
             type:'GET',
             dataType:'json',
             success:function(resultado) {
                 if (resultado.status == true){
    ...
    function alterarUnidadeNegocio() {
         $.magnificPopup.open({
                removalDelay: 500,
                items: {
                  src: '#alterarUnidadeNegocio'
                },
                callbacks: {
    ...
    function alterarSelectUnidadeNegocio()
   {
       $("#selectUsuario").append();
       $("#selectUsuario").multiselect('destroy');
       $('#selectUsuario').multiselect({
           includeSelectAllOption: true,
           enableCaseInsensitiveFiltering: true,
           enableFiltering: true,
...
function formatedToNumber(number)
{
   if (isNaN(number) == true) {
      number = number.split("R$").join('');
